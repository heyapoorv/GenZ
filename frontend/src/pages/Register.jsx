    ...
    <main className="pt-[120px] pb-section-gap min-h-screen flex items-center justify-center px-margin-mobile">
      <div className="w-full max-w-md bg-surface-container border border-outline-variant/10 p-8">
        <div className="text-center mb-8">
          <h1 className="font-headline-lg text-[32px] uppercase mb-2">Request Access</h1>
          <p className="font-body-md text-on-surface-variant">Join the Zenith Elite Program.</p>
        </div>
        
        {message && <div className="bg-error/10 border border-error/20 text-error p-3 mb-6 font-body-md text-sm">{message}</div>}
        ...
        <form onSubmit={submitHandler} className="space-y-6">
          ...
          <div className="space-y-2">
            <label className="font-label-caps text-xs text-on-surface-variant uppercase">Confirm Password</label>
            <input 
              type="password" 
              required
        ...
        <div className="mt-8 pt-6 border-t border-outline-variant/10 text-center">
          <p className="font-body-md text-sm text-on-surface-variant">
            Already have clearance? <Link to={redirect ? `/login?redirect=${redirect}` : '/login'} className="text-primary hover:underline">Sign In</Link>
          </p>
        </div>
      </div>
    </main>
